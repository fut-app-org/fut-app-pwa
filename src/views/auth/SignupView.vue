...
<template>
  <div class="mx-auto flex min-h-dvh w-full max-w-md flex-col bg-bg">
    <header
      class="px-6 pb-[22px] pt-16 text-white"
      style="background-image: linear-gradient(150deg, #0a3b28, #0f5238)"
    >
    ...
    <form class="flex flex-1 flex-col gap-4 px-6 py-[22px]" @submit.prevent="submit">
      ...
      <label class="flex flex-col gap-1.5">
        ...
        <input v-model="password" required type="password" minlength="8" autocomplete="new-password" class="input" placeholder="Mínimo 8 caracteres" />
      </label>

      <p v-if="error" class="rounded-xl bg-dangerBg px-4 py-3 text-[13px] font-medium text-danger">{{ error }}</p>

      <div class="mt-auto flex flex-col gap-2.5 pb-6">
