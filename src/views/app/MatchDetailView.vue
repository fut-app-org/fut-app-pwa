...
async function vote(category: 'top_scorer' | 'worst_player', candidateId: string) {
  ...
  error.value = ''
  try {
    await api.post(`/matches/${matchId}/votes`, { category, candidate_id: candidateId })
    await load()
  } catch (e) {
...
async function uploadMedia(event: Event) {
  ...
  try {
    ...
    await api.post(`/matches/${matchId}/media`, form)
    await load()
  } catch (e) {
    error.value = errorMessage(e)
  } finally {
    ...
    <template #header>
      <div class="flex items-center gap-2.5">
        ...
        <div v-if="match">
          ...
          <div class="text-[13px] text-white/65">{{ match.venue }} · {{ participants.length }} jogadores</div>
        </div>
      </div>
    </template>

    <div v-if="detail && match" class="flex flex-col gap-3 px-4 pt-3.5">
      <!-- Aviso de votação -->
      <div
        v-if="votingOpen"
        ...
        <div class="mt-2.5 flex flex-col gap-3">
          <div v-for="team in detail.teams" :key="team.id">
            <div class="mb-1.5 flex items-center gap-2">
              <span class="h-3.5 w-3.5 rounded-full" :style="{ backgroundColor: team.team_color }" />
      ...
      <Card class="p-4">
        ...
        <div class="grid grid-cols-4 gap-2">
          <div v-for="item in detail.media" :key="item.id" class="group relative aspect-square overflow-hidden rounded-[10px] bg-surface2">
            <img v-if="item.type === 'photo'" :src="item.url" :alt="item.caption || 'Foto da partida'" class="h-full w-full object-cover" loading="lazy" />
            <video v-else :src="item.url" class="h-full w-full object-cover" preload="metadata" controls />
            <button
